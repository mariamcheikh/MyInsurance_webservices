...
$result=mysqli_query($conn,"INSERT INTO `imagecamera` (  `image`,`constat_id`) VALUES ( 'image',$constat_id)");

	$fetchId=mysqli_query($conn,"SELECT `id` FROM `imagecamera`");
	$id=1;
	while($rowid=mysqli_fetch_array($fetchId,MYSQLI_ASSOC)){
		$id=$rowid['id'];

	}
	$filePath = "imagesCamera/image$id.png";#name of the file and the location where the image is to be updated
	#full URL of the file (used to download the image during login)
	$url = "image$id.png";
	$update=mysqli_query($conn,"UPDATE `imagecamera` SET `image`='$url' WHERE id='$id'");#updating the profile-pic url
	if($update){
